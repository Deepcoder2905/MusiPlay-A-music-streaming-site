...
def admin_login():
    ...
    input_username = input_data.get('username')  # Updated to use 'username'
    input_password = input_data.get('password')

    if not input_username or not input_password:
        return jsonify({'message': 'Username and Password are required.'}), 400

    if input_username != 'admin':  # Validate username instead of email
        return jsonify({'message': 'Invalid Username'}), 400

    user = datastore.find_user(username=input_username)  # Find user by username
...
def user_login():
    input_data = request.get_json()
    input_username = input_data.get('username')  # Updated to match frontend naming
    input_password = input_data.get('password')

    if not input_username or not input_password:
        return jsonify({'message': 'Username and Password are required.'}), 400
    
    # Replace email checks with username where applicable
    if input_username == 'admin':
        return jsonify({'message': 'Invalid Username'}), 400
    
    user = datastore.find_user(username=input_username)  # Updated to use username
    ...
    if user.active == False:
        return jsonify({'message': 'Your account has been blacklisted.'}), 401
